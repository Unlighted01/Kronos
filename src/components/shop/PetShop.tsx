...
import { db, initPetStats, SHOP_ITEMS, ShopCatalogItem, InventoryItem } from '../../db/kronosDb';
import { Award, ShoppingBag, Check, Zap, Heart } from 'lucide-react';
import { audioSynth } from '../../utils/audioSynth';

export const PetShop: React.FC = () => {
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'snack' | 'furniture' | 'wallpaper'>('all');
  ...
  const handleUseItem = async (item: InventoryItem) => {
    ...
    const newHappiness = Math.min(100, happiness + (item.statBoost.happiness || 0));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-950/60 border border-slate-800 rounded-2xl p-5">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Spend Kronos Coins earned from focus sessions to buy snacks & 2D platformer themes!
          </p>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="bg-amber-500/10 border border-amber-500/20 px-4 py-2 rounded-xl flex items-center space-x-2">
            ...
            <div>
              <span className="text-[10px] text-amber-300 font-pixel block">WALLET</span>
              <span className="text-lg font-bold text-amber-400">{coins} Coins</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-800 pb-3">
        {(['all', 'snack', 'furniture', 'wallpaper'] as const).map((cat) => (
          <button
            key={cat}
            onClick={() => {
          ...
          >
            {cat === 'all' ? 'All Items' : cat === 'snack' ? 'Snacks & Treats' : cat === 'furniture' ? '2D Furniture' : 'Wallpapers'}
          </button>
        ))}
      </div>
                  ...
                  <div className="flex items-center space-x-2 mb-3">
                    {item.statBoost.energy && (
                      <span className="flex items-center space-x-1 text-[10px] text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                        <Zap size={10} />
                        <span>+{item.statBoost.energy} Energy</span>
                      </span>
                    )}
                    {item.statBoost.happiness && (
                      <span className="flex items-center space-x-1 text-[10px] text-pink-400 bg-pink-500/10 px-2 py-0.5 rounded border border-pink-500/20">
                        <Heart size={10} />
                        <span>+{item.statBoost.happiness} Joy</span>
                      </span>
                    )}
                  </div>
                )}
      ...
      {inventory && inventory.length > 0 && (
        <div className="pt-4">
          <h3 className="text-sm font-bold text-slate-200 mb-3">Your Inventory & Treats</h3>
            ...
            {inventory.map((invItem) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  <span className="text-2xl">{invItem.icon}</span>
                  <div>
                    <h4 className="text-xs font-bold text-slate-200">{invItem.name}</h4>
                    <span className="text-[9px] text-slate-500 uppercase">{invItem.category}</span>
                  </div>
                </div>
